...
$(function(){
	...
	$("#submitButton").on("click",function(){
		...
		for(var i=0; i<data.length; i++){
			...
			var num = Number($(data[i]).parent().parent().find("input").last().val());
			if(!/^(([0-9]*[1-9][0-9]*$)|(0))$/.test(num)){
				$.messager.alert("提示","请正确输入数字!");
				return;
			}
			if(num>0){
				var one = {};
				var goodsDetailId = $(data[i]).attr("goodsDetailId");
				one.goodsDetailId=goodsDetailId;
				one.useNum=num;
				parts[j]=one;
				j++;//数组计数，防止null元素
			}
		...
		if(j==0){
			$.messager.confirm('项目进度', '当前没有部件申请使用，您是否继续保存？', function (yes) {
				if(yes == true){
	...
	function sumUseNum(obj){
		...
		if(num > dispatchNum){
			if(newUseNum>0){
				showMessager('使用数量','总使用数量超过调度数量:'+dispatchNum+',当前已使用量:'+useNum);
			}
			if(dispatchNum > useNum){
				$(obj).val(dispatchNum - useNum);
			}else{
				$(obj).val(0);
			}
		}
        ...
        <table  id="mainPartTable">
          <thead>
            <tr>
              ...
              <th scope="col" >需求</th>
              <th scope="col" >调度</th>
              <th scope="col" style="display: none;">使用</th>
              <th scope="col" >使用</th>
              <th scope="col" >单位</th>
              <th scope="col" ></th>
            </tr>
          ...
          <tbody>
           ...
           <tr pid="${part.pid }" partid="${part.part_model_id }" >
              ...
              <td style="display: none;">${part.useNum }</td>
              <td>
              <c:choose>
              <c:when test="${part.partNumber > part.useNum }">
              <c:choose>
              <c:when test="${part.dispatchNum > part.useNum }">
              <input value="${part.dispatchNum - part.useNum }" onblur="sumUseNum(this);"/>
              </c:when>
              <c:otherwise>
              <input value="0" onblur="sumUseNum(this);"/>
              </c:otherwise>
              </c:choose>
              </c:when>
              <c:otherwise>
 			   		  	...
 			   		  	<c:choose>
						       <c:when test="${partThree.part_model_pid==partTow.part_model_id}">
 						            <tr class="dataTr" pid="${partThree.part_model_pid }" partid="${partThree.part_model_id }" >
							              ...
							              <td style="display: none;">${partThree.useNum }</td>
							              <td>
							              <c:choose>
							              <c:when test="${partThree.partNumber > partThree.useNum }">
							              <c:choose>
							              <c:when test="${partThree.dispatchNum > partThree.useNum }">
							              <input value="${partThree.dispatchNum - partThree.useNum }" onblur="sumUseNum(this);"/>
							              </c:when>
							              <c:otherwise>
							              <input value="0" onblur="sumUseNum(this);"/>
							              </c:otherwise>
