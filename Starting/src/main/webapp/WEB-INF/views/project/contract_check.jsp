...
<SCRIPT type=text/javascript>
	function onProjectSubmit(){
		$.messager.confirm('合同审核', '合同信息审核之后不能再编辑合同内容，你确定继续吗？', function (yes) {
			if(yes == true){
				$.ajax({
					async : false,
					cache : false,
					...
					url :'<%=basePath %>contractPro/checkSave',
					error : function() {// 请求失败处理函数
						$.messager.alert('错误','保存请求提交失败!','error');
					},
					success : function(data){
						if('n' == data){
							$.messager.alert('失败','合同信息审核失败!','error');
						}else{
							$.messager.alert('成功','合同信息审核成功!','info',function(){
        ...
        <form id="conProForm" method="post">
          ...
          <table border="0" class="edit-table">
            <tr>
              ...
              <c:when test="${con.contractStatus == 2 }">完成</c:when>
          ...
          <table  class="edit-table" border="0">
            <tr>
              ...
              <a onclick="onProjectSubmit();" class="button_new" title="保存"><img src="<%=basePath %>static/images/icons/check_icons.png" class="px18"/></a>
              </td>
            </tr>
          </table>
        </form>
