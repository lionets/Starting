                ...
                if(tatongMethod.isChecked()){
                    ...
                    if(params.length==0){
                        if($('#tab22222').hasClass('current')){
                            tatongMethod.alert("系统提示",'必须配置权限信息','error');
                        }else{
                            $('#tab22222').parent().siblings().find("a").removeClass('current');
                            $('#tab22222').addClass('current');
                            var currentTab = $('#tab22222').attr('href');
                            $(currentTab).siblings().hide();
                            $(currentTab).show();
                        }
                    }else{
                        tatongMethod.confirm("修改","确定要保存吗?",function(){
                            $('input[name="powerIds"]').val(params.join(','));
                            $.ajax({
                                type: "POST",
                                url:'<%=basePath%>role/addOne',
                                async: true,
                                data:$('#form').serialize(),
                                ...
                                success: function(data) {
                                    ...
                                    if (success) {
                                        tatongMethod.confirm("系统提示",'保存成功,要返回到查询列表吗?',function(){
...
<div id="main-content">
    ...
    <div class="content-box">
        ...
        <form id="form" action="<%=basePath %>role/addOne" method="post">
        ...
        <table>
            <tr>
                ...
                <a class="button_new" href="javascript:void(0)" id="submitButton" title="保存"><img  src="<%=basePath %>static/images/icons/save-icon.png" class="px18"/></a> </td>
            </tr>
        </table>
