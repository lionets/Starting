	...
	$(document).ready(function() {
		...
		$("#submitButton").on("click",function(){
			if(tatongMethod.isChecked()){
				//验证唯一
				if($("#dicval").val().toUpperCase()==$("#dictValue").val().toUpperCase()){
		...
		if("${dic.dictParentId}"=="null"||"${dic.dictParentId}"==""){
			//$("#iscz2").attr("checked",true);
			$("#cztr").remove();
			$("#iscz").val(1);
			iscz="1";
		}
...
<div id="main-content">
  ...
  <div class="content-box">
    ...
    <div class="content-box-content">
      <div class="tab-content default-tab" id="tab1">
        <form action="updatePublic" id="form" method="post">
        ...
        <input type="hidden" id="iscz" name="iscz">
        <input type="hidden" id="dicval" value="${dic.dictValue}">      
          <table border="0" class="edit-table">
            <!-- <tr>
              <th><label ><b>*</b>词组类别</label></th>
		      <td>
		      	<input type="radio" id="iscz1" name="iscz" value="0">否<input type="radio" id="iscz2" name="iscz" value="1">是
		      </td>
		      <td class="zhushi">&nbsp;</td>
		    </tr> -->
            <tr id="cztr">
    ...
    <td>
          	<select name="czlb" id="wordClass" class="easyui-combobox">
					...
					<c:forEach var="cz"  items="${czlist}" varStatus="status" >
             			<option <c:if test="${dic.dictParentId==cz.id }">selected="selected"</c:if> value="${cz.id }">${cz.dictValue }</option>
             		</c:forEach>
			   </select>           
     </td>
    <td class="zhushi">&nbsp;</td>
  ...
  <tr>
    <th scope="row"><label><b>*</b>词组中文名称</label></th>
    <td><input id="dictValue" name="dictValue" required="true" type="text"  value="${dic.dictValue }"/></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <th scope="row"><label><b>*</b>词组繁体中文名称</label></th>
    <td><input name="dictValueT" required="true" type="text" value="${dic.dictValueT }"/></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <th scope="row"><label><b>*</b>词组英文名称</label></th>
    <td><input name="dictValueE" required="true" type="text" value="${dic.dictValueE }"/></td>
    <td>&nbsp;</td>
  </tr>
  <%-- 
  <tr>
    <th scope="row">最后修改时间
      <label></label></th>
    <td><input name="modifyTime" type="text" readonly="readonly"  value="<fmt:formatDate  value='${publicInfo.modify_time }'   pattern='yyyy-MM-dd HH:mm:ss'/>"/></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
  ...
  <tr>
    <th scope="row">描述</th>
    <td><textarea name="dictMemo" cols="" rows="" >${dic.dictMemo }</textarea>
      &nbsp;</td>
    <td>&nbsp;</td>
  </tr>
<tr>
