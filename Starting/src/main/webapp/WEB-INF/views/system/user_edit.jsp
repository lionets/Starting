...
function companyListView(selectid, param){
	...
	$(document).ready(function() {
		...
		$(".addOneDuty").on("click",function(){
			...
			var duty_type = $( "option[value='"+id+"']").attr("duty_type");
			//已经存在的不容许再添加
            ...
            "<td><div align='center'>"+duty_type+"</div></td>"+
            "<td><div align='center'>"+
        ...
        <input type="hidden" name="id" value="${comuserinfo.id }">
          <table border="0" class="edit-table">
            <tr>
              ...
              <td><input required="true"  name="accountName" readonly="readonly" value="${comuserinfo.accountName }" type="text" /></td>
            ...
            <tr>
              ...
              <td><input type="password"  vtype="limit" minLength="6" maxLength="14" required="true" id="accountPassword"  name="accountPassword" value="${accountPassword }"/></td>
               <th scope="row"><label><b>*</b>确认密码</label></th>
              <td><input  type="password" vtype='passwordAgain' name="accountPasswordAgain" id="accountPasswordAgain" value="${accountPassword }"/></td>
            </tr>
            <tr>
             <th scope="row">手机</th>
            ...
            <tr>
              ...
              <td>
              		<select   name="AccountStatus"  id="AccountStatus" class="easyui-combobox">
										<option value="1" <c:if test="${comuserinfo.accountStatus==1 }">selected="selected"</c:if>>可登陆</option>
										<option value="4" <c:if test="${comuserinfo.accountStatus==4 }">selected="selected"</c:if>>不可登陆</option>
					</select></td>
            </tr>
            </table>
            <input type="hidden" name="dutyIds" id="dutyIds">
				    ...
				    <c:forEach var="duty"  items="${dutyLists}" varStatus="status" >
						<c:choose>
							<c:when test="${comuserinfo.dutyid==comuserinfo.id }">
								<option value="${duty.id }" duty_type="${duty.duty_type }" selected="selected">${duty.duty_name }</option>
							</c:when>
							<c:otherwise>
