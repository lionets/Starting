...
using(['window']);//加载window插件
	$(document).ready(function() {
		...
		$("#submitButton").on("click",function(){
				if("${comCompanyInfo.companyPid}" == "0"){//如果不是顶级公司
					var pid = $("#companyPid").combobox("getValue");
					if(pid =="0"){
						tatongMethod.alert("提示","请选择上级公司！");
						return;
					}
				}
...
<div id="main-content">
  ...
  <div class="content-box">
    ...
    <div class="content-box-content">
      <div class="tab-content default-tab" id="tab1">
        <form  action="<%=basePath %>company/updateCompany?id=${id}&tab=tab1&tab1PageNum=${tab1PageNum}&tab1PerPage=${tab1PerPage}" id="form" method="post">
    ...
    <th scope="row"><label ><b>*</b>公司代码</label></th>
     <td><input required="true"  name="companyCode" type="text"  value="${comCompanyInfo.companyCode}"/></td>
  </tr>
  ...
  <tr>
    ...
    <c:forEach var="company"  items="${plist}" varStatus="status" >
		   <option value="${company.id }" <c:if test="${company.id==companyPid }">selected="selected"</c:if>>${company.companyShortName }</option>
   </c:forEach>
   </select>
    </td>
 <th scope="row">机构类型</th>
  ...
  <tr>
    ...
    <td><input required="true"  vtype="telPhone"  name="companyTel" type="text" value="${comCompanyInfo.companyTel}"/>
</td>
  </tr>
