...
$(function(){
	...
	$("#submitButton").on("click",function(){
		...
		for(var i=0; i<trs.length; i++){
			...
			var id = $(oneTr).attr("partModelId")==null?"":$(oneTr).attr("partModelId");
...
<div id="main-content">
  ...
  <div class="content-box">
    ...
    <div class="content-box-content">
      <div class="tab-content default-tab" id="tab1">
        <form action="<%=basePath %>part/updatePart" method="post">
    ...
    <td>  <select class="easyui-combobox"  name="level" style="width:200px;">
						<option value="1" <c:if test='${thisLevel.level==1 }'>selected="selected" </c:if>>主部件</option>
						<option value="2" <c:if test='${thisLevel.level==2 }'>selected="selected" </c:if>>零部件</option>
				</select>
     </td>
    ...
    <td>
    		<select  name="part_unit" id="part_unit" class="easyui-combobox">
    			<c:forEach var="part_unit" items="${part_units }">
    					<option value="${part_unit.dict_value }" <c:if test="${part_unit.dict_value==thisLevel.part_unit }">selected="selected"</c:if>>${part_unit.dict_name }</option>
    			</c:forEach>
    		</select>
    </td>
           <th scope="row"><label><b>*</b>英文简称</label></th>
	    <td>
	    	<input name="part_letter" required="true" vtype="english" value="${thisLevel.part_letter }">
	    </td>
  </tr>
  <tr>
    <th scope="row">描述</th>
  ...
  <table style=" width:500px">
            ...
            <c:forEach var="partModel" items="${partModelList }" varStatus="status">
                <tr class="alt-row" partModelId="${partModel.id }">
	                <td><div align="center">
	            	  ${status.index+1}
	                </div></td>
	                <td><div align="center"><input name="Input2" type="text" value="${partModel.part_model }"></div></td>
	                <td><div align="center"><input name="Input2" type="text" value="${partModel.memo }"></div></td>
              ...
              <tr>
                <td><a class="button1 button_position" title="编辑"><img src="<%=basePath %>static/images/icons/add_icon.png" class="px18"></a></td>
                <td colspan="10">&nbsp;</td>
              </tr>
            </tbody>
