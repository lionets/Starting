...
using(['window'],function(){});//加载window插件
	$(document).ready(function() {
				...
				//验证重复
				var maintain_no = $('[name="maintain_no"]').val();
				$("#submitButton").on("click",function(){
					if(tatongMethod.isChecked()){
					    ...
					    $.ajax( {
					        dataType : "JSON",
					        type : "POST",
					        url : "<%=basePath%>/maintain/checkMaintainRp",
					        data : "maintain_no="+maintain_no,
					        success : function(countStr) {
					        	...
					        	if(count == 0){
					        		...
					        		var html = '<span class="apendhtml" style="color: red;margin-left: 5px;">维修单编号重复</span>';
					        		$('[name="maintain_no"]').after(html);
					        	}
					        },
					        error : function(html) {
  ...
  <div class="content-box">
    ...
    <div class="content-box-content">
      <div class="tab-content default-tab" id="tab1">
        <form action="<%=basePath %>maintain/addMaintain" method="post">
        <input type="hidden" name="tab2PageNum" value="0">
        <input type="hidden" name="id" value="${id }">
          <table border="0" class="edit-table">
            <tr>
              <th><label ><b>*</b>维修单编号</th>
        <td><input name="maintain_no" type="text" required="true" value="${maintainList.maintain_no }"/></td>
        <td class="zhushi">&nbsp;</td>
      </tr>
      <tr >
        <th>维修类别
          <label></label></th>
        <td><input name="maintain_type" type="radio" value="1" checked="checked"/>
          大修
          <input name="maintain_type" type="radio" value="2" />
          项修</td>
        <td>&nbsp;</td>
      </tr>
      <tr >
    ...
    <tr >
        <th>维修资产数量</th>
        <td><input name="number" vtype='integer' value="${maintainList.number }" required="true" type="text" /></td>
        <td>&nbsp;</td>
      </tr>
      <tr >
        <th>维修内容</th>
