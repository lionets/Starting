  ...
  <div class="content-box">
    ...
    <div class="tab-content default-tab" id="tab1">
        <form action="<%=basePath %>handle/addHandle" method="post">
        	...
        	<input type="hidden" name="tower_no" id="tower_no"/>
          ...
          <table border="0" class="edit-table">
            <tr>
              ...
              <td class="px200"><label>
                 <input value="towerRadio" checked="checked" name="manageType"  type="radio" />塔机 
  				<input value="partRadio" name="manageType"    type="radio">部件
              </label></td>
...
<%--                		<option  value="转让" <c:if test='${disposal_type=="转让" }'>selected="selected"</c:if>>转让</option>
					...
					<c:forEach var="zcczfs" items="${zcczfsList }">
                               <option  value="${zcczfs.dict_name }"  <c:if test='${disposal_type==zcczfs.dict_name }'>selected="selected"</c:if>>${zcczfs.dict_value }</option>
     				</c:forEach>
              </select>
                  </td>
              <th ><label><b>*</b>处置时间</label></th>
              <td ><input name="disposal_date" type="text" required="true"  readonly="readonly"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});"/></td>
              <td></td>
            </tr>
           
          </table>
          <p>
            <label> <b> 资产信息</b><label style="padding-left:61px"> <b style="color:#FF0000">*</b>发出地</label><label id="startAddressLabel" style="padding-left:20px"></label>
        </p>
        <div >
          <table width="100%" >
            <thead>
...
function openwin(){
	...
	window.open('<%=basePath%>handle/add_part?towerModel='+towerModel+'&towerNo='+towerNo+"&come_address="+come_address,'newwindow','height=700,width=1200,top=50,left=100,toolbar=0,menubar=0,scrollbar=yes,resizable=no,location=no,status=no')
}</script>
</html>
