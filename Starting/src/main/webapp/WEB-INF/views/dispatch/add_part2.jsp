...
$(function(){
			...
			$("tr[pid='"+partid+"']").each(function(){
				...
				if(!isNaN(dynamicNum) && dynamicNum >= 1){
					...
					input.val(dynamicNum);
 					var maxNum = input.parent().parent().prev().text();
	...
	$("#submitButton").on("click",function(){
			...
			$("#dynamicTbody",pdocument).children().children(":eq(2)").children().text("${towerModel}");
			$("#tower_model",pdocument).val("${towerModel}");
			window.close();
	})
            ...
            <th >塔机型号:</th>
              <td>
					...
					<c:forEach var="oneTowerModel"  items="${towerModelList}" varStatus="status" >
								<option value="${oneTowerModel.tower_model }"  <c:if test="${oneTowerModel.tower_model==towerModel }">selected="selected"</c:if>>${oneTowerModel.tower_model }</option>
					</c:forEach>
				</select>
                  </td>
