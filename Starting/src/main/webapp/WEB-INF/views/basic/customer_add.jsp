...
function checkValue(obj){
	...
	if(!/^[\w]+$/.test(value) && isEmpty(value) == false){
		...
		$.ajax({
			...
			success : function(data){
				if('n' == data){
					$.messager.alert('编号重复','输入的客户编号已存在，请输入唯一编号!','error');
					$(obj).val('');
				}
			}
		});
        ...
        <form id="addCustomerForm" action="" method="post">
          <table border="0" class="edit-table">
            <tr>
              ...
              <td><input class="medium-input" type="text" onblur="checkValue(this);" id="customerCode" name="customerCode" required="true"/></td>
              <td>(输入不能超过20个字)</td>
            </tr>
            <tr>
