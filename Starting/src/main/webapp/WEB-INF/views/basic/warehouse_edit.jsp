	...
	$(document).ready(function() {
		...
		$('#dispatchId').combobox({
			...
			onLoadSuccess:function(){
				$('#dispatchId').combobox('setValue','${info.manageCompanyId}');
			}
		});
		$('#repArea').combobox({
			...
			onLoadSuccess:function(){
				$('#repArea').combobox('setValue','${info.repArea}');
			}
		});
		$('#repProvince').combobox({
			...
			onLoadSuccess:function(){
				if(1 > loadNum){//第一次页面初始化时才加载
					$('#repProvince').combobox('setValue','${info.repProvince}');
				}
			}
		});
		...
		$('#repCity').combobox({
			...
			onLoadSuccess:function(){
				if(1 > loadNum){//第一次页面初始化时才加载
					$('#repCity').combobox('setValue','${info.repCity}');
				}
			}
		});
		...
		$("#submitButton").on("click",function(){
			if(tatongMethod.isChecked()){
				$.ajax({
					...
					url :'<%=basePath %>warehouse/editSave',
        ...
        <form id="repForm" method="post">
          <table border="0" class="edit-table">
            <tr>
              ...
              <td><input name="storeName" type="text" value="${info.storeName }" required="true"/>
            ...
            <tr>
              ...
              <td><input name="repContact" type="text" value="${info.repContact }" required="true"/></td>
              <td><span >(输入不能超过20个字)</span></td>
            </tr>
            <tr>
              <th scope="row">电话</th>
              <td>
                <input name="rermark3" type="text" value="${info.rermark3 }"/></td>
              <td>&nbsp;</td>
            </tr>
            <tr>
              <th scope="row">手机</th>
              <td><input name="repConPhone" type="text" value="${info.repConPhone }"/></td>
            ...
            <tr>
              ...
              <input name="repAddress" type="text" class="medium-input" value="${info.repAddress }" required="true"/>
            </td>
              <td></td>
            </tr>
            <tr>
              <th scope="row"><label ><b>*</b>仓库面积</label></th>
              <td><input name="repRermark1" type="text" value="${info.repRermark1 }" required="true"/> </td>
              <td>平方米(m2)</td>
            </tr>
            <tr>
              <th scope="row">道路交通</th>
              <td><textarea name="repRermark2" id="rep_rermark2" cols="25" rows="3">${info.repRermark2 }</textarea></td>
              <td></td>
            </tr>
            <tr>
              <th scope="row">管理公司</th>
              <td>
              <select id="dispatchId" name="manageCompanyId"></select>
              </td>
              <td></td>
            </tr>
            <tr>
              <th scope="row">租期</th>
              <td><input name="repCreateDate" type="text" readonly="readonly" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${info.repCreateDate}" type="both" />"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})"/></td>
              <td></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
